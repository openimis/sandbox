...
class Query(ExportableQueryMixin, graphene.ObjectType):
    ...
    insuree_by_uuid_public = graphene.Field(
        InsureeGQLType,
        uuid=graphene.UUID(required=True),  # validated by Graphene
        description="Public endpoint: fetch a single insuree by UUID (limited fields).",
    )

    def resolve_insuree_by_uuid_public(self, info, uuid):
        # `uuid` here is a uuid.UUID instance already
        return (
            Insuree.objects
            .filter(uuid=uuid, validity_to__isnull=True)
...
class Mutation(graphene.ObjectType):
    ...
    exchange_code = ExchangeCodeMutation.Field()
    user_info = UserInfoMutation.Field()
    send_verification_email = SendVerificationEmailMutation.Field()
...
def on_code_change_mutation(kwargs):
    return 'on_family_and_insuree_mutation(kwargs)'   
