...
use App\Http\Controllers\FhirPatientController;
use App\Http\Controllers\FhirInsuranceClaimController;
use App\Http\Controllers\FhirEndpointController;
...
Route::middleware('auth')->group(function () {
    ...
    // Settings
    // Route::resource('patients', PatientController::class)->except(['show', 'edit', 'create']);

    // // Doctor routes
    // Route::resource('doctors', DoctorController::class)->except(['show', 'edit']);
    ...
    Route::get('/clinic/insurance/patient-billing', function () {
        return Inertia::render('PatientBilling/Index');
    })->name('patient.billing.index');

    Route::get('/clinic/insurance/claim-history', [FhirInsuranceClaimController::class, 'index'])->name('patients.claims');

    Route::post('/fhir/check-patient-eligibility', [FhirPatientController::class, 'checkPatientEligibility'])
        ->name('fhir.patient.checkEligibility');
    Route::post('/fhir/claim/submit', [FhirInsuranceClaimController::class, 'submitClaim'])->name('fhir.claim.submit');
    Route::post('/fhir/coverage/checkEligibility', [FhirInsuranceClaimController::class, 'checkEligibility'])->name('fhir.coverage.checkEligibility');
    Route::get('api/fhir/claim/update/{claimUuid}', [FhirInsuranceClaimController::class, 'fetchAndUpdateClaim']);
    Route::resource('fhir-endpoints', FhirEndpointController::class)->middleware(['auth']);
